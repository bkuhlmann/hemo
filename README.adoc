...
See the Hanamismith link:https://alchemists.io/projects/hanamismith/#_demonstrations[demonstration] for details.

== Requirements

. link:https://www.ruby-lang.org[Ruby].
. {postgres_link}.
. {overmind_link}: Optional but recommended for launching your local web server.
...
As fun as this simple demonstration application is, there are a few outstanding issues:

- There is a link:https://bugs.ruby-lang.org/issues/19363#change-101540[Ruby Issue] regarding a segmentation fault when attempting to build this app while using link:https://github.com/simplecov-ruby/simplecov[SimpleCov] with evaluation enabled.
- There is an issue with running Google Chrome (headless) on Circle CI which is why the build keeps failing due to using an link:https://alchemists.io/projects/docker-alpine-ruby[Alpine Linux Docker image].
- Configuring link:https://sequel.jeremyevans.net/[Sequel] with the link:https://github.com/DatabaseCleaner/database_cleaner-sequel[Database Cleaner] doesn't always fully clean up the database so running link:https://github.com/guard/guard[Guard] or link:https://rspec.info/[RSpec] locally and/or on CI might intermittently fail.
...
== link:https://alchemists.io/projects/hemo/versions[Versions]
